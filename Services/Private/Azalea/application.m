...
saveIconNameFor(char *iconPath, char *wm_instance, char *wm_class)
{
    ...
    NSMutableString *name = AUTORELEASE([[NSMutableString alloc] init]);
    id value;

    if (wm_instance)
      [name appendString: [NSString stringWithCString: wm_instance]];
    if (wm_class)
    {
      if ([name length] > 0)
	[name appendString: @"."];

      [name appendString: [NSString stringWithCString: wm_class]];
    }

    value = [defaults objectForKey: WAIcon window: name];

    if (value == nil)
    {
      value = [NSString stringWithCString: iconPath];
      [defaults setObject: value forKey: WAIcon window: name];
    }

    if (value && !wPreferences.flags.noupdates) {
...
wApplicationCreate(WWindow *wwin)
{
...
#if 0 // #ifdef USER_MENU // FIXME: removed
    ...
    if (wapp->app_icon) {
        ...
        if (tmp && strstr(tmp, "Library/WindowMaker/CachedPixmaps")!=NULL &&
            ...
            wmessage(("recreating missing icon '%s'"), tmp);
