...
updateSettingsPanelIcon(AppSettingsPanel *panel)
{
    ...
    if (!file)
        ...
        if (!path) {
            wwarning(("could not find icon %s, used in a docked application"),
                     file);
            wfree(file);
            WMSetLabelImage(panel->iconLabel, NULL);
            return;
...
panelBtnCallback(WMWidget *self, void *data)
{
    ...
    if (panel->okBtn == btn) {
        ...
        if (!wIconChangeImageFile(panel->editedIcon->icon, text)) {
            ...
            buf = wmalloc(len);
            snprintf(buf, len, ("Could not open specified icon file: %s"), text);
            if (wMessageDialog(panel->wwin->screen_ptr, ("Error"), buf,
                               ("OK"), ("Ignore"), NULL) == WAPRDefault) {
                if (text)
                    wfree(text);
                wfree(buf);
                return;
...
ShowDockAppSettingsPanel(WAppIcon *aicon)
{
    ...
    WMSetFrameTitle(panel->commandFrame, ("Application path and arguments"));
    ...
    WMSetFrameTitle(panel->pasteCommandFrame,
                    ("Command for middle-click launch"));
    ...
    WMSetFrameTitle(panel->dndCommandFrame,
                    ("Command for files dropped with DND"));
    ...
    WMSetButtonText(panel->browseBtn, ("Browse..."));
...

    {
        ...
        WMSetButtonText(panel->cancelBtn, ("Cancel"));
