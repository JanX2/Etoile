...
print_help()
{
    ...
    puts(("The Window Maker window manager for the X window system"));
    puts("");
    puts((" -display host:dpy	display to use"));
#ifdef USECPP
    puts((" --no-cpp 		disable preprocessing of configuration files"));
#endif
    puts((" --no-dock		do not open the application Dock"));
    puts((" --no-clip		do not open the workspace Clip"));
    puts((" --no-autolaunch	do not autolaunch applications"));
    puts((" --dont-restore		do not restore saved session"));

    puts((" --locale locale	locale to use"));

    puts((" --create-stdcmap	create the standard colormap hint in PseudoColor visuals"));
    puts((" --visual-id visualid	visual id of visual to use"));
    puts((" --static		do not update or save configurations"));
    puts((" --no-polling		do not periodically check for configuration updates"));
#ifdef DEBUG
    puts((" --synchronous		turn on synchronous display mode"));
#endif
...
main(int argc, char **argv)
{
    ...
    WMApplication *app = [WMApplication wmApplication];
    [app initializeWithName: [NSString stringWithCString: "WindowMaker"]
...
//    WMInitializeApplication("WindowMaker", &argc, argv);
    ...
    if (i_am_the_monitor)
      return_value = MonitorLoop(argc, argv);
    else
      return_value = real_main(argc, argv);
            ...
            if (strcmp(argv[i], "-nocpp")==0
                ...
                if (strcmp(argv[i], "--for-real")==0) {
                    ...
                    if (sscanf(argv[i], "%i", &wVisualID)!=1) {
                        wwarning(("bad value for visualid: \"%s\""), argv[i]);
                        exit(0);
                    }
