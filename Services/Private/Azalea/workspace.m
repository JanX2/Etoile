...
wWorkspaceNew(WScreen *scr)
{
    ...
    if (scr->workspace_count < MAX_WORKSPACES) {
        ...
        if (!wspace->name) {
            wspace->name = wmalloc(strlen(("Workspace %i"))+8);
            sprintf(wspace->name, ("Workspace %i"), scr->workspace_count);
        }
...
wWorkspaceRename(WScreen *scr, int workspace, char *name)
{
    ...
    if (strlen(tmp)==0) {
        snprintf(buf, sizeof(buf), ("Workspace %i"), workspace+1);
    } else {
...
wWorkspaceSaveState(WScreen *scr)
{
  ...
  NSArray *old_wks_state = [NSArray arrayWithArray: [defaults workspacesForScreen: scr->screen]];
  NSMutableArray *parr = AUTORELEASE([[NSMutableArray alloc] init]);
  NSMutableDictionary *wks_state = nil;
  id pstr = nil, foo = nil, bar = nil;
  int i;

  for (i = 0; i < scr->workspace_count; i++) {
    pstr = [NSString stringWithCString: scr->workspaces[i]->name];
    wks_state = AUTORELEASE([[NSMutableDictionary alloc] init]);
    [wks_state setObject: pstr forKey: dName];
    if (!wPreferences.flags.noclip) {
      pstr = wClipSaveWorkspaceState(scr, i);
      [wks_state setObject: pstr forKey: dClip];
    } else if ([old_wks_state count]) {
      foo = [old_wks_state objectAtIndex: i];
      if (foo != nil) {
	bar = [(NSDictionary *)foo objectForKey: dClip];
	if (bar != nil) {
          [wks_state setObject: bar forKey: dClip];
	}
      }
    }
    [parr addObject: wks_state];
  }
  [defaults setWorkspaces: parr forScreen: scr->screen];
}
...
{
  WMDefaults *defaults = [WMDefaults sharedDefaults];
  NSArray *parr = [defaults workspacesForScreen: scr->screen];
  NSDictionary *wks_state;
  id pstr = nil, clip_state = nil;
  int i, j, count, wscount;

  if ((parr == nil) || ([parr count] == 0))
    return;

  wscount = scr->workspace_count;
  count = ([parr count] < MAX_WORKSPACES) ? [parr count] : MAX_WORKSPACES;
  for (i = 0; i < count; i++) {
    wks_state = [parr objectAtIndex: i];
    pstr = [wks_state objectForKey: dName];
    if (i >= scr->workspace_count)
      wWorkspaceNew(scr);
    wfree(scr->workspaces[i]->name);
    scr->workspaces[i]->name = wstrdup((char*)[pstr cString]);
    if (!wPreferences.flags.noclip) {
      clip_state = [wks_state objectForKey: dClip];
