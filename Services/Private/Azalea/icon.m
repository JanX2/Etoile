...
wIconCreateWithIconFile(WScreen *scr, char *iconfile, int tile)
{
    ...
    if (iconfile) {
        ...
        if (!icon->image) {
            wwarning(("error loading image file \"%s\""), iconfile, RMessageForError(RErrorCode));
        }
...
makeIcon(WScreen *scr, RImage *icon, int titled, int shadowed, int tileType)
{
    ...
    if (!RConvertImage(scr->rcontext, tile, &pixmap)) {
        wwarning(("error rendering image:%s"), RMessageForError(RErrorCode));
    }
    RReleaseImage(tile);
...
getnameforicon(WWindow *wwin)
{
    ...
    NSLog(@"ps %@", ps);
    NSString *p = [[ps objectAtIndex: 0] stringByAppendingPathComponent: @"WindowMaker"];
    //prefix = wusergnusteppath();
    prefix = (char*)[p cString];
    ...
    if (access(path, F_OK)!=0) {
        if (mkdir(path, S_IRUSR|S_IWUSR|S_IXUSR)) {
            wsyserror(("could not create directory %s"), path);
            wfree(path);
            wfree(suffix);
            return NULL;
        }
        ...
        if (icon->image) {
            ...
            if (!scr->def_icon_pixmap) {
                ...
                if (file) {
                    path = FindImage(wPreferences.icon_path, file);
                    if (!path) {
                        wwarning(("could not find default icon \"%s\""),file);
                        goto make_icons;
                    }

                    image = RLoadImage(scr->rcontext, path, 0);
                    if (!image) {
                        wwarning(("could not load default icon \"%s\":%s"),
