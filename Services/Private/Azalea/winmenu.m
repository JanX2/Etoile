...
updateMakeShortcutMenu(WMenu *menu, WWindow *wwin)
{
    ...
    for (i=WO_ENTRIES; i<smenu->entry_no; i++) {
        ...
        snprintf(buffer, buflen, "%s %i", ("Set Shortcut"), shortcutNo+1);
...
makeWorkspaceMenu(WScreen *scr)
{
    ...
    if (!menu) {
        wwarning(("could not create submenu for window menu"));
        return NULL;
    }
...
makeOptionsMenu(WScreen *scr)
{
    ...
    entry = wMenuAddCallback(menu, ("Keep on top"), execWindowOptionCommand,
                             NULL);
    entry->flags.indicator = 1;
    entry->flags.indicator_type = MI_CHECK;

    entry = wMenuAddCallback(menu, ("Keep at bottom"), execWindowOptionCommand,
                             NULL);
    entry->flags.indicator = 1;
    entry->flags.indicator_type = MI_CHECK;

    entry = wMenuAddCallback(menu, ("Omnipresent"), execWindowOptionCommand,
                             NULL);
    entry->flags.indicator = 1;
    entry->flags.indicator_type = MI_CHECK;
...
createWindowMenu(WScreen *scr)
{
     ...
     * this file.
     */
    entry = wMenuAddCallback(menu, ("Maximize"), execMenuCommand, NULL);
    ...
    entry = wMenuAddCallback(menu, ("Miniaturize"), execMenuCommand, NULL);
    ...
    entry = wMenuAddCallback(menu, ("Shade"), execMenuCommand, NULL);
    ...
    entry = wMenuAddCallback(menu, ("Hide"), execMenuCommand, NULL);
    ...
    entry = wMenuAddCallback(menu, ("Resize/Move"), execMenuCommand, NULL);
    ...
    entry = wMenuAddCallback(menu, ("Select"), execMenuCommand, NULL);
    ...
    entry = wMenuAddCallback(menu, ("Move To"), NULL, NULL);
    scr->workspace_submenu = makeWorkspaceMenu(scr);
    if (scr->workspace_submenu)
        wMenuEntrySetCascade(menu, entry, scr->workspace_submenu);

    entry = wMenuAddCallback(menu, ("Attributes..."), execMenuCommand, NULL);

    entry = wMenuAddCallback(menu, ("Options"), NULL, NULL);
    wMenuEntrySetCascade(menu, entry,
                         makeMakeShortcutMenu(scr, makeOptionsMenu(scr)));

    /*
     entry = wMenuAddCallback(menu, ("Select Shortcut"), NULL, NULL);
     wMenuEntrySetCascade(menu, entry, makeMakeShortcutMenu(scr));
     */

    entry = wMenuAddCallback(menu, ("Close"), execMenuCommand, NULL);
    ...
    entry = wMenuAddCallback(menu, ("Kill"), execMenuCommand, NULL);
...
updateMenuForWindow(WMenu *menu, WWindow *wwin)
{
    ...
    if (wwin->flags.miniaturized) {
        ...
        if (!text) text = ("Deminiaturize");

        menu->entries[MC_MINIATURIZE]->text = text;
    } else {
        static char *text = NULL;
        if (!text) text = ("Miniaturize");
