...
/* WMaker */
WScreen*
wScreenWithScreenNumber(int n)
{
    int i;
    for (i=0; i<wScreenCount; i++) {
        if (wScreen[i]->screen == n) {
            return wScreen[i];
        }
    }
...
StartUp(Bool defaultScreenOnly)
{
    ...
    /* initialize defaults stuff and read static defaults */
    [WMDefaults sharedDefaults];

    /* check sanity of some values */
    if (wPreferences.icon_size < 16) {
        wwarning(("icon size is configured to %i, but it's too small. Using 16, instead\n"),
    ...
    if (!WDWindowAttributes->dictionary) {
        wwarning(("could not read domain \"%s\" from defaults database"),
                 "WMWindowAttributes");
    }
#endif
    ...
    if(wPreferences.modelock && !wXkbSupported) {
        wwarning(("XKB is not supported. KbdModeLock is automatically disabled."));
    ...
    for (j = 0; j < max; j++) {
        if (defaultScreenOnly || max==1) {
            ...
            if (!wScreen[wScreenCount]) {
                wfatal(("it seems that there is already a window manager running"));
                Exit(1);
            }
        } else {
            wScreen[wScreenCount] = wScreenInit(j);
            if (!wScreen[wScreenCount]) {
                wwarning(("could not manage screen %i"), j);
                continue;
            }
        }
        wScreenCount++;
...
//        wMenuRestoreState(wScreen[j]);
    ...
    if (wScreenCount == 0) {
        wfatal(("could not manage any screen"));
        Exit(1);
    }
