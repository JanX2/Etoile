...
{
    if (!value) {
        wwarning(("could not define value for %s for cpp"), name);
        return;
    }
    strcat(line, name);
    strcat(line, value);
...
MakeCPPArgs(char *path)
{
    ...
    *line = 0;
    ...
    if ((buf=getenv("HOSTNAME"))!=NULL) {
        if (buf[0]=='(') {
            wwarning(("your machine is misconfigured. HOSTNAME is set to %s"),
                     buf);
        } else
            putdef(line, " -DHOST=", buf);
    } else if ((buf=getenv("HOST"))!=NULL) {
        if (buf[0]=='(') {
            wwarning(("your machine is misconfigured. HOST is set to %s"),
                     buf);
        } else
            putdef(line, " -DHOST=", buf);
    }
...
ExpandOptions(WScreen *scr, char *cmdline)
{
    ...
    out = malloc(olen);
    if (!out) {
        wwarning(("out of memory during expansion of \"%s\""));
        return NULL;
    }
    *out = 0;
        ...
        case S_OPTION:
            ...
            case 'w':
                if (scr->focused_window
                    ...
                    if (!nout) {
                        wwarning(("out of memory during expansion of \"%w\""));
                        goto error;
                    }
                    out = nout;
                    strcat(out,tmpbuf);
            ...
            case 'a':
                ...
                if (user_input) {
                    ...
                    nout = realloc(out,olen);
                    if (!nout) {
                        wwarning(("out of memory during expansion of \"%a\""));
                        goto error;
                    }
                    out = nout;
            ...
            case 'd':
                ...
                nout = realloc(out,olen);
                if (!nout) {
                    wwarning(("out of memory during expansion of \"%d\""));
                    goto error;
                }
                out = nout;
            ...
            case 's':
                ...
                if (!selection) {
                    wwarning(("selection not available"));
                    goto error;
                }
...
void
ParseWindowName(NSString *value, char **winstance, char **wclass, char *where)
{
    char *name;
    ...
    *winstance = *wclass = NULL;
    ...
    if ((value == nil) || ([value length] == 0)){
        NSLog(@"Warning: bad window name value in %s state info", where);
        return;
    }

    name = (char*)[value cString];
...
SendHelperMessage(WScreen *scr, char type, int workspace, char *msg)
{
    ...
    if (write(scr->helper_fd, buffer, len+4) < 0) {
        wsyserror(("could not send message to background image helper"));
    }
    wfree(buffer);
}
