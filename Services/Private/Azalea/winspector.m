            ...
            char *wm_instance, char *wm_class, int flags)
{
    ...
    if (file) {
        ...
        if (!path) {
            ...
            buf = wmalloc(len);
            snprintf(buf, len, ("Could not find icon \"%s\" specified for this window"),
                     file);
            wMessageDialog(panel->frame->screen_ptr, ("Error"), buf,
                           ("OK"), NULL, NULL);
            wfree(buf);
            wfree(file);
            return -1;
        ...
        if (!pixmap) {
            ...
            buf = wmalloc(len);
            snprintf(buf, len, ("Could not open specified icon \"%s\":%s"),
...
getBool(WMPropList *value)
{
    ...
    if ((val[1]=='\0' && (val[0]=='y' || val[0]=='Y' || val[0]=='T'
        ...
        wwarning(("can't convert \"%s\" to boolean"), val);
        return 0;
    }
}
#endif
...
static int
insertAttribute(NSMutableDictionary *window, NSString *attr, id value, int flags)
{
...
saveSettings(WMButton *button, InspectorPanel *panel)
{
    ...
    if (icon_file) {
        if (icon_file[0] != 0) {
	    value = [NSString stringWithCString: icon_file];
            different |= insertAttribute(winDic, WAIcon, value, flags);
            different2 |= insertAttribute(appDic, WAIcon, value, flags);
        }
        wfree(icon_file);
    }

    {
	    ...
	    value = [NSString stringWithCString: panel->frame->screen_ptr->workspaces[i]->name];
            different |= insertAttribute(winDic, WAStartWorkspace, value,
                                         flags);
        }
    }
    ...
    value = (WMGetButtonSelected(panel->alwChk)!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAAlwaysUserIcon, value, flags);

    value = (WMGetButtonSelected(panel->attrChk[0])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoTitlebar,     value, flags);

    value = (WMGetButtonSelected(panel->attrChk[1])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoResizebar,     value, flags);

    value = (WMGetButtonSelected(panel->attrChk[2])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoCloseButton,   value, flags);

    value = (WMGetButtonSelected(panel->attrChk[3])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoMiniaturizeButton, value, flags);

    value = (WMGetButtonSelected(panel->attrChk[4])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoBorder, value, flags);

    value = (WMGetButtonSelected(panel->attrChk[5])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAKeepOnTop, value, flags);

    value = (WMGetButtonSelected(panel->attrChk[6])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAKeepOnBottom, value, flags);

    value = (WMGetButtonSelected(panel->attrChk[7])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAOmnipresent, value, flags);

    value = (WMGetButtonSelected(panel->attrChk[8])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAStartMiniaturized, value, flags);

    value = (WMGetButtonSelected(panel->attrChk[9])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAStartMaximized,  value, flags);

    value = (WMGetButtonSelected(panel->attrChk[10])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAFullMaximize,  value, flags);

    value = (WMGetButtonSelected(panel->moreChk[0])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoKeyBindings,   value, flags);

    value = (WMGetButtonSelected(panel->moreChk[1])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoMouseBindings, value, flags);

    value = (WMGetButtonSelected(panel->moreChk[2])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WASkipWindowList, value, flags);

    value = (WMGetButtonSelected(panel->moreChk[3])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAUnfocusable, value, flags);

    value = (WMGetButtonSelected(panel->moreChk[4])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAKeepInsideScreen,value, flags);

    value = (WMGetButtonSelected(panel->moreChk[5])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoHideOthers,    value, flags);

    value = (WMGetButtonSelected(panel->moreChk[6])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WADontSaveSession, value, flags);

    value = (WMGetButtonSelected(panel->moreChk[7])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WAEmulateAppIcon,  value, flags);

#ifdef XKB_BUTTON_HINT
    value = (WMGetButtonSelected(panel->moreChk[8])!=0) ? WAYes : WANo;
    different |= insertAttribute(winDic, WANoLanguageButton, value, flags);
#endif

    if (wwin->main_window!=None && wApplicationOf(wwin->main_window)!=NULL) {
        value = (WMGetButtonSelected(panel->appChk[0])!=0) ? WAYes : WANo;
        different2 |= insertAttribute(appDic, WAStartHidden, value, flags);

        value = (WMGetButtonSelected(panel->appChk[1])!=0) ? WAYes : WANo;
        different2 |= insertAttribute(appDic, WANoAppIcon, value, flags);

        value = (WMGetButtonSelected(panel->appChk[2])!=0) ? WAYes : WANo;
        different2 |= insertAttribute(appDic, WASharedAppIcon, value, flags);
    }
	...
	if ([key2 isEqualToString: name]) {
	    ...
	    NSString *key2 = [NSString stringWithFormat: @"%s.%s",
		     instance, class];
...
selectSpecification(WMWidget *bPtr, void *data)
{
    ...
    snprintf(str, len, ("Inspecting  %s.%s"),
                         ...
                         Bool showSelectPanel)
{
    ...
    spec_text = ("The configuration will apply to all\n"
    ...
    WMSetButtonText(panel->saveBtn, ("Save"));
    ...
    WMSetButtonText(panel->applyBtn, ("Apply"));
    ...
    WMSetButtonText(panel->revertBtn, ("Reload"));
    ...
    WMAddPopUpButtonItem(panel->pagePopUp, ("Window Specification"));
    WMAddPopUpButtonItem(panel->pagePopUp, ("Window Attributes"));
    WMAddPopUpButtonItem(panel->pagePopUp, ("Advanced Options"));
    WMAddPopUpButtonItem(panel->pagePopUp, ("Icon and Initial Workspace"));
    WMAddPopUpButtonItem(panel->pagePopUp, ("Application Specific"));

    /**** window spec ****/
    frame_width = PWIDTH - (2 * 15);

    panel->specFrm = WMCreateFrame(panel->win);
    WMSetFrameTitle(panel->specFrm, ("Window Specification"));
    ...
    WMSetButtonText(panel->defaultRb, ("Defaults for all windows"));
    ...
    WMSetButtonText(panel->selWinB, ("Select window"));
    ...
    WMSetFrameTitle(panel->attrFrm, ("Attributes"));
    ...
    for (i=0; i < 11; i++) {
        ...
        switch (i) {
        case 0:
            caption = ("Disable titlebar");
            flag = WFLAGP(wwin, no_titlebar);
            descr = ("Remove the titlebar of this window.\n"
            ...
            break;
        case 1:
            caption = ("Disable resizebar");
            flag = WFLAGP(wwin, no_resizebar);
            descr = ("Remove the resizebar of this window.");
            break;
        case 2:
            caption = ("Disable close button");
            flag = WFLAGP(wwin, no_close_button);
            descr = ("Remove the `close window' button of this window.");
            break;
        case 3:
            caption = ("Disable miniaturize button");
            flag = WFLAGP(wwin, no_miniaturize_button);
            descr = ("Remove the `miniaturize window' button of the window.");
            break;
        case 4:
            caption = ("Disable border");
            flag = WFLAGP(wwin, no_border);
            descr = ("Remove the 1 pixel black border around the window.");
            break;
        case 5:
            caption = ("Keep on top (floating)");
            flag = WFLAGP(wwin, floating);
            descr = ("Keep the window over other windows, not allowing\n"
                      "them to cover it.");
            break;
        case 6:
            caption = ("Keep at bottom (sunken)");
            flag = WFLAGP(wwin, sunken);
            descr = ("Keep the window under all other windows.");
            break;
        case 7:
            caption = ("Omnipresent");
            flag = WFLAGP(wwin, omnipresent);
            descr = ("Make window present in all workspaces.");
            break;
        case 8:
            caption = ("Start miniaturized");
            flag = WFLAGP(wwin, start_miniaturized);
            descr = ("Make the window be automatically miniaturized when it's\n"
                      "first shown.");
            break;
        case 9:
            caption = ("Start maximized");
            flag = WFLAGP(wwin, start_maximized!=0);
            descr = ("Make the window be automatically maximized when it's\n"
                      "first shown.");
            break;
        case 10:
            caption = ("Full screen maximization");
            flag = WFLAGP(wwin, full_maximize);
            descr = ("Make the window use the whole screen space when it's\n"
    ...
    WMSetFrameTitle(panel->moreFrm, ("Advanced"));
    ...
    for (i=0;
        ...
        switch (i) {
        case 0:
            caption = ("Do not bind keyboard shortcuts");
            flag = WFLAGP(wwin, no_bind_keys);
            descr = ("Do not bind keyboard shortcuts from Window Maker\n"
                      "when this window is focused. This will allow the\n"
                      "window to receive all key combinations regardless\n"
                      "of your shortcut configuration.");
            break;
        case 1:
            caption = ("Do not bind mouse clicks");
            flag = WFLAGP(wwin, no_bind_mouse);
            descr = ("Do not bind mouse actions, such as `Alt'+drag\n"
                      "in the window (when alt is the modifier you have\n"
                      "configured).");
            break;
        case 2:
            caption = ("Do not show in the window list");
            flag = WFLAGP(wwin, skip_window_list);
            descr = ("Do not list the window in the window list menu.");
            break;
        case 3:
            caption = ("Do not let it take focus");
            flag = WFLAGP(wwin, no_focusable);
            descr = ("Do not let the window take keyboard focus when you\n"
                      "click on it.");
            break;
        case 4:
            caption = ("Keep inside screen");
            flag = WFLAGP(wwin, dont_move_off);
            descr = ("Do not allow the window to move itself completely\n"
                      "outside the screen. For bug compatibility.\n");
            break;
        case 5:
            caption = ("Ignore 'Hide Others'");
            flag = WFLAGP(wwin, no_hide_others);
            descr = ("Do not hide the window when issuing the\n"
                      "`HideOthers' command.");
            break;
        case 6:
            caption = ("Ignore 'Save Session'");
            flag = WFLAGP(wwin, dont_save_session);
            descr = ("Do not save the associated application in the\n"
                      "session's state, so that it won't be restarted\n"
                      "together with other applications when Window Maker\n"
                      "starts.");
            break;
        case 7:
            caption = ("Emulate application icon");
            flag = WFLAGP(wwin, emulate_appicon);
            descr = ("Make this window act as an application that provides\n"
                      "enough information to Window Maker for a dockable\n"
                      "application icon to be created.");
            break;
#ifdef XKB_BUTTON_HINT
        case 8:
            caption = ("Disable language button");
            flag = WFLAGP(wwin, no_language_button);
            descr = ("Remove the `toggle language' button of the window.");
            break;
#endif
        }
    ...
    WMSetFrameTitle(panel->iconFrm, ("Miniwindow Image"));
    ...
    WMSetButtonText(panel->browseIconBtn, ("Browse..."));
    ...
    WMSetLabelText(panel->fileLbl, ("Icon filename:"));
    ...
    WMSetButtonText(panel->alwChk, ("Ignore client supplied icon"));
    WMSetButtonSelected(panel->alwChk, WFLAGP(wwin, always_user_icon));
    ...
    WMSetFrameTitle(panel->wsFrm, ("Initial Workspace"));

    WMSetBalloonTextForView(("The workspace to place the window when it's"
    ...
    WMAddPopUpButtonItem(panel->wsP, ("Nowhere in particular"));
    ...
    if (wwin->main_window != None) {
        ...
        WMSetFrameTitle(panel->appFrm, ("Application Attributes"));
        ...
        for (i=0; i < 3; i++) {
            ...
            case 0:
                ...
                flag = WFLAGP(wapp->main_window_desc, start_hidden);
                descr = ("Automatically hide application when it's started.");
                break;
            case 1:
                caption = ("No application icon");
                flag = WFLAGP(wapp->main_window_desc, no_appicon);
                descr = ("Disable the application icon for the application.\n"
                          "Note that you won't be able to dock it anymore,\n"
                          "and any icons that are already docked will stop\n"
                          "working correctly.");
                break;
            case 2:
                caption = ("Shared application icon");
                flag = WFLAGP(wapp->main_window_desc, shared_appicon);
                descr = ("Use a single shared application icon for all of\n"
