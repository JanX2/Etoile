...
wScreenInit(int screen_number)
{
    ...
    if (!scr->rcontext) {
        wwarning(("could not initialize graphics library context: %s"),
                 RMessageForError(RErrorCode));
        wAbort(False);
    } else {
        char **formats;
    ...
    if (!scr->wmscreen) {
        wfatal(("could not initialize WINGs widget set"));
        return NULL;
    }
...
wScreenRestoreState(WScreen *scr)
{
    ...
    if (!wPreferences.flags.nodock) {
        state = [defaults dockForScreen: scr->screen];
        scr->dock = wDockRestoreState(scr, state, WM_DOCK);
    }

    if (!wPreferences.flags.noclip) {
        state = [defaults clipForScreen: scr->screen];
...
wScreenSaveState(WScreen *scr)
{
    ...
    old_state = [defaults sessionStatesForScreen: scr->screen];

    /* save dock state to file */
    if (!wPreferences.flags.nodock) {
	// FIXME: this may be old format 
        wDockSaveState(scr, old_state);
    } else {
	/* FIXME: don't need to move
	foo = [defaults dockForScreen: scr->screen];
	if (foo != nil) {
	  [defaults setDock: foo forScreen: scr->screen];
        }
	*/
    }
    if (!wPreferences.flags.noclip) {
	...
	/* FIXME: don't need to move 
	foo = [defaults clipForScreen: scr->screen];
	if (foo != nil) {
	  [defaults setClip: foo forScreen: scr->screen];
        }
	*/
    }
