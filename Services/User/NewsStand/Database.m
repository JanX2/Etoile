		...
		if (pathToPList != nil)
		{
			...
			if (demoFeedsDict)
			{
				...
				while ((feedName = [enumerator nextObject]) != nil)
				{
					...
					NSString * feedURL = [itemDict objectForKey:@"URL"];
...
{
	return [fieldsByName objectForKey:name];
}
		...
		switch ([field type])
		{
			...
			case MA_FieldType_Folder: {
				...
				switch ([criteria operator])
				{
					...
					default:					NSAssert(NO, @"Invalid operator for folder field type");
				}
...
{
NSLog(@"%@ %@:%d", self, NSStringFromSelector(_cmd), folderId);
	NSMutableArray * newArray = [NSMutableArray array];
	...
	if (folderId == 0)
	{
		if ([filterString isEqualToString:@""] == NO)
	...
	if (results && [results rowCount])
	{
		...
		// own count of unread is in sync with the folders count and fix
		// them if not.
		if (folder && [filterString isEqualToString:@""] && IsRSSFolder(folder))
		{
			if (unread_count != [folder unreadCount])
			{
				NSLog(@"Fixing unread count for %@ (%d on folder versus %d in articles)", [folder name], [folder unreadCount], unread_count);
...
NSLog(@"%@ %@:%d done", self, NSStringFromSelector(_cmd), folderId);
	// Deallocate
	[results release];
	return newArray;
}
