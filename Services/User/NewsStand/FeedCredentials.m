...
-(void)credentialsForFolder:(NSWindow *)window folder:(Folder *)aFolder
{
	...
	// Set the focus
	[[self window] makeFirstResponder:([[folder username] isBlank]) ? userName : password];

	[self enableOKButton];
	[NSApp beginSheet: [self window] modalForWindow:window modalDelegate:nil didEndSelector:NULL contextInfo:nil];
}
