...
-(id)initWithFrame:(NSRect)frame
{
    if (([super initWithFrame:frame]) != nil)
	{
		...
		/* build UI */
		NSRect rect = frame;
		splitView2 = [[NSSplitView alloc] initWithFrame: frame];
	    [splitView2 setAutoresizingMask: NSViewWidthSizable|NSViewHeightSizable];
		[splitView2 setVertical: NO];
	    ...
	    [articleText setAutoresizingMask: NSViewWidthSizable|NSViewHeightSizable];
		[scrollView setDocumentView: articleList];
		[splitView2 addSubview: scrollView];
		[splitView2 addSubview: articleText];
...
//		NSRect mainFrame = [[splitView2 superview] frame];
		NSRect mainFrame = [splitView2 frame];
...
{
	if ([controller primaryView] == self)
...
-(void)ensureSelectedArticle:(BOOL)singleSelection
{
	...
	{
		int index = [articleList selectedRow];
		if ((index < 0) || (index == NSNotFound))
			[self makeRowSelectedAndVisible:0];
		else
			[articleList scrollRowToVisible:index];
	}
}
...
-(void)updateVisibleColumns
{
	...
	for (index = 0; index < count; ++index)
	{
		...
		if (showField)
		{
			...
			{
				[(NSTextFieldCell *)[column dataCell] setDrawsBackground:NO];
				[(NSTextFieldCell *)[column dataCell] setWraps:YES];
			}
...
-(void)selectFolderWithFilter:(int)newFolderId
{
	...
	[articleController sortArticles];
	[articleList reloadData];
	if ((guidOfArticleToSelect == nil) && ([[articleController allArticles] count]))
	{
...
-(void)refreshArticleAtCurrentRow:(BOOL)delayFlag
{
	if ((currentSelectedRow < 0) || (currentSelectedRow == NSNotFound))
	{
...
- (void) setController: (AppController *) _controller
{
    controller = _controller;
}

- (void) setArticleController: (ArticleController *) _articleController
{
    articleController = _articleController;
}

- (void) setArticleView: (ArticleView *) _articleText
{
    articleText = _articleText;
}
    
- (void) setFoldersTree: (FoldersTree *) _foldersTree 
{
    foldersTree = _foldersTree;
}
