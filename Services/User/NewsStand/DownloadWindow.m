...
-(id)init
{
	NSRect rect = NSMakeRect(200, 200, 300, 400);;
	...
	[column setWidth: rect.size.width];
...
#else
	[table setColumnAutoresizingStyle: NSTableViewFirstColumnOnlyAutoresizingStyle];
...
//	[table sizeToFit];
	...
	rect = NSMakeRect(5, 5, 100, 18);
	clearButton = [[NSButton alloc] initWithFrame: rect];
    [clearButton setAutoresizingMask: NSViewMaxXMargin|NSViewMaxYMargin];
	[clearButton setTarget: self];
	[clearButton setAction: @selector(clearList:)];
	[clearButton setBezelStyle: NSRoundedBezelStyle];
	[[clearButton cell] setControlSize: NSMiniControlSize];
	[[win contentView] addSubview: clearButton];
	[clearButton release];
...
-(void)windowDidLoad
{
	...
	[[self window] setTitle:NSLocalizedString(@"Downloads", nil)];
}
