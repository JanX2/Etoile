...
-(Article *)articleFromGuid:(NSString *)guid
{
	if (isCached == NO)
	{
		NSLog(@"FIXEM (%@ %@): Folder's cache of articles should be initialized before articleFromGuid can be used", self, NSStringFromSelector(_cmd));
...
-(void)removeArticleFromCache:(NSString *)guid
{
	if (isCached == NO)
	{
		NSLog(@"FIXEM (%@ %@): Folder's cache of articles should be initialized before removeArticleFromCache can be used", self, NSStringFromSelector(_cmd));
		[[Database sharedDatabase] arrayOfArticles:itemId filterString:nil];
	}
