...
static BOOL get_prealloc(Display *display, Window win, Atom prop, Atom type, int size, unsigned char *data, unsigned long num)
{
    BOOL ret = NO;
    int res;
...
static BOOL get_all(Display *display, Window win, Atom prop, Atom type, int size, unsigned char **data, unsigned int *num)
{
    BOOL ret = NO;
    int res;
    unsigned char *xdata = NULL;
    ...
    res = XGetWindowProperty(display, win, prop, 0l, LONG_MAX,
                             FALSE, type, &ret_type, &ret_size,
                             &ret_items, &bytes_left, &xdata);
    if (res == Success) {
...
static BOOL get_stringlist(Display *display, Window win, Atom prop, char ***list, int *nstr)
{
    XTextProperty tprop;
    BOOL ret = NO;

    if (XGetTextProperty(display, win, &tprop, prop) && tprop.nitems) {
...
{
    return get_prealloc(display, win, prop, type, 32, (unsigned char*)ret, 1);
}

BOOL prop_get_array32(Display *display, Window win, Atom prop, Atom type, unsigned long **ret, unsigned int *nret)
{
    return get_all(display, win, prop, type, 32, (unsigned char**)ret, nret);
}
...
BOOL prop_get_string_locale(Display *display, Window win, Atom prop, NSString **ret)
{
    ...
    if (get_stringlist(display, win, prop, &list, &nstr) && nstr) {
...
BOOL prop_get_strings_locale(Display *display, Window win, Atom prop, NSArray **ret)
{
    ...
    if (get_all(display, win, prop, prop_atoms.string, 8, (unsigned char**)&raw, &num)) {
...
BOOL prop_get_string_utf8(Display *display, Window win, Atom prop, NSString **ret)
{
    ...
    if (get_all(display, win, prop, prop_atoms.utf8, 8, (unsigned char**)&raw, &num)) {
...
void prop_set_array32(Display *display, Window win, Atom prop, Atom type, unsigned long *val,
                      unsigned int num)
{
    XChangeProperty(display, win, prop, type, 32, PropModeReplace,
...
{
    XChangeProperty(display, win, prop, prop_atoms.utf8, 8,
