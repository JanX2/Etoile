		...
		while (!_shouldFinish) {
			...
			if (_asynchronous && (-[_metronome timeIntervalSinceNow] >= 1.0)) {
				...
				if (shouldContinue) {
					...
					[_progressDelegate setDonePerTotalMessage:[NSString stringWithFormat:@"%d/%@", _numberOfDoneLeaves, (_numberOfTotalLeaves <= 0? (id)@"???" : (id)[NSString stringWithFormat:@"%d", _numberOfTotalLeaves])]];
				}
				[_metronome release];
				_metronome = [[NSDate alloc] init];
...
// aBranch == nil: resume from a break
{
	...
	if (_leafProcessing == nil && !_exceptionRaised) {
		...
		if (_enumeratorProcessing != nil) {
			...
			[_progressDelegate setDonePerTotalMessage:[NSString stringWithFormat:@"%d/%@", _numberOfDoneLeaves, (_numberOfTotalLeaves == -1? (id)@"???" : (id)[NSString stringWithFormat:@"%d", _numberOfTotalLeaves])]];
